...
enum Accounting {
    ...
    static let account = DocType(
        ...
        fields: [
            ...
            FieldDefinition(key: "account_number", label: "Account Number",
                            type: .text, required: false,
                            helpText: "Numeric code for this account (e.g. 1010). Used for ordering and accountant reports.",
                            isSearchable: true),
            ...
            FieldDefinition(key: "root_type", label: "Root Type",
                            ...
                            options: ["", "Asset", "Liability", "Equity",
                                      "Income", "Expense"]),
            FieldDefinition(key: "normal_balance", label: "Normal Balance",
                            type: .select, required: false,
                            helpText: "The side this account normally carries. Assets and expenses are Debit; liabilities, equity, and income are Credit.",
                            options: ["", "Debit", "Credit"]),
            FieldDefinition(key: "is_tax_control", label: "Tax Control Account",
                            type: .boolean, required: false, defaultValue: .bool(false),
                            helpText: "Marks VAT / GST / sales-tax control accounts for tax reporting."),
    ...
    static let glEntry = DocType(
        ...
        fields: [
            ...
            FieldDefinition(key: "currency", label: "Currency",
                            type: .link, required: false, linkedDocType: "Currency"),
            FieldDefinition(key: "conversion_rate", label: "Conversion Rate",
                            type: .decimal, required: false, defaultValue: .double(1)),
            FieldDefinition(key: "base_debit", label: "Debit (Base)",
                            type: .currency, required: false, defaultValue: .double(0)),
            FieldDefinition(key: "base_credit", label: "Credit (Base)",
                            type: .currency, required: false, defaultValue: .double(0)),
            FieldDefinition(key: "party_type", label: "Party Type",
                            type: .select, required: false,
                            options: ["", "Customer", "Supplier", "Employee"]),
    ...
    static let custTrans = DocType(
        ...
        fields: [
            ...
            FieldDefinition(key: "conversion_rate", label: "Conversion Rate",
                            type: .decimal, required: false, defaultValue: .double(1)),
            FieldDefinition(key: "base_amount", label: "Amount (Base)",
                            type: .currency, required: false, defaultValue: .double(0)),
            FieldDefinition(key: "voucher_type", label: "Voucher Type",
                            type: .text, required: true),
            FieldDefinition(key: "voucher_no", label: "Voucher No",
